...
static void encountered_newline() { ++loc.ln; }
...
void token_kind_print(TOK kind) {
  printf("`");
  switch (kind) {
  case TOK_EOI: printf("End of Input"); break;
  case TOK_BR: printf("BR"); break;
  case TOK_COMMA: printf(","); break;
  case TOK_COLON: printf(":"); break;
  case TOK_INTLIT: printf("Integer Literal"); break;
  case TOK_LARROW: printf("<-"); break;
  case TOK_LBL: printf("Label"); break;
  case TOK_NL: printf("Newline"); break;
  case TOK_PLUS: printf("+"); break;
  case TOK_PRINT: printf("PRINT"); break;
  case TOK_REG: printf("Register"); break;
  }
  printf("`");
}
...
void token_print(Token tok) {
  switch (tok.kind) {
  case TOK_INTLIT: printf("`Lit: %d`", tok.val); break;
  case TOK_LBL: printf("`.%d`", tok.val); break;
  case TOK_REG: printf("`%%%d`", tok.val); break;
  default:
...
static int is_midchar(char c) { return isalpha(c) || c == '_'; }
...
static int is_token(TOK kind) { return (token.kind == kind); }

static int match_token(TOK kind) {
  if (is_token(kind)) {
    next_token();
    return 1;
  }
...
static void expect_token(TOK kind) {
  if (!match_token(kind)) {
...
int starts_value(void) {
  return (is_token(TOK_REG) || is_token(TOK_INTLIT));
}
...
Instruction parse_instruction(int bb_num, CFG cfg) {
  ...
  case TOK_BR:
  {
    ...
    if (match_token(TOK_LBL)) {
      ...
      cfg_add_edge(cfg, bb_num, lbl1);
      cfg_add_edge(cfg, bb_num, lbl2);
    }
  } break;
  default:
...
void parse_bb(CFG cfg) {
  ...
  while (starts_instruction()) {
    buf_push(cfg.bbs[bb_num].insts, parse_instruction(bb_num, cfg));
  }
}
...
CFG parse_procedure(int *max_reg) {
  ...
  if (nbbs) {
    cfg_construct_reserve(&cfg, nbbs);
    int i = 0;
